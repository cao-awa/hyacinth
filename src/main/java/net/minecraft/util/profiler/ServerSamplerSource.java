...
implements SamplerSource {
    ...
    public static Set<Sampler> createSystemSamplers() {
        ...
        try {
            ...
            IntStream.range(0, cpuUsageFetcher.logicalProcessorCount).mapToObj(index -> Sampler.create("cpu#" + index, SampleType.CPU, () -> cpuUsageFetcher.getCpuUsage(index))).forEach(builder::add);
        }
        catch (Throwable cpuUsageFetcher) {
            LOGGER.warn("Failed to query cpu, no cpu stats will be recorded", cpuUsageFetcher);
        }
        builder.add(Sampler.create("heap MiB", SampleType.JVM, () -> (float)(Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()) / 1048576.0f));
