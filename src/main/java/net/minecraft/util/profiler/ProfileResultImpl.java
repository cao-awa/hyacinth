...
implements ProfileResult {
    ...
    private static final Comparator<Map.Entry<Object, Object>> COMPARATOR = Map.Entry.comparingByValue(Comparator.comparingLong(counterInfo -> ((CounterInfo)counterInfo).totalTime)).reversed();
    ...
    private Map<String, CounterInfo> setupCounters() {
        ...
        this.locationInfos.forEach((location, info) -> {
            ...
            if (!object2LongMap.isEmpty()) {
                ...
                object2LongMap.forEach((marker, count) -> map.computeIfAbsent(marker, k -> new CounterInfo()).add(list.iterator(), count));
            }
        });
        return map;
    }
    ...
    private void appendTiming(int level, String name, StringBuilder sb) {
        ...
        object2LongMap.forEach((marker, count) -> ProfileResultImpl.indent(sb, level).append('#').append(marker).append(' ').append(count).append('/').append(count / (long)this.tickDuration).append('\n'));
        if (list.size() < 3) {
            return;
        }
    ...
    private static String generateWittyComment() {
        ...
        try {
            return strings[(int)(TimeUtil.measuringTimeNano() % (long)strings.length)];
        }
        catch (Throwable throwable) {
    ...
    static class CounterInfo {
        long selfTime;
        long totalTime;
        final Map<Object, Object> subCounters = Maps.newHashMap();
        ...
        public void add(Iterator<String> pathIterator, long time) {
            ...
            if (!pathIterator.hasNext()) {
                ...
                ((CounterInfo)this.subCounters.computeIfAbsent(pathIterator.next(), k -> new CounterInfo())).add(pathIterator, time);
            }
        }
    }
}
