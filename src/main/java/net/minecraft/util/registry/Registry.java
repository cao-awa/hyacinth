...
//    public static final RegistryKey<Registry<LootScoreProviderType>> LOOT_SCORE_PROVIDER_TYPE_KEY = Registry.createRegistryKey("loot_score_provider_type");
     ...
     * @see #DIMENSION_KEY
     * @see World#OVERWORLD
     * @see com.github.cao.awa.hyacinth.server.MinecraftServer#worlds
     */
...
//    public static final Registry<StructurePoolElementType<?>> STRUCTURE_POOL_ELEMENT = Registry.create(STRUCTURE_POOL_ELEMENT_KEY, () -> StructurePoolElementType.EMPTY_POOL_ELEMENT);
    ...
    public static <T extends MutableRegistry<?>> void validate(MutableRegistry<T> registry) {
        registry.forEach(mutableRegistry2 -> {
            if (mutableRegistry2.getIds().isEmpty()) {
                EntrustExecution.error("Registry '" + registry.getId(mutableRegistry2) + "' was empty after loading");
            }
    ...
    private static <T, R extends MutableRegistry<T>> R create(RegistryKey<? extends Registry<T>> key, R registry, Supplier<T> defaultEntry, Lifecycle lifecycle) {
        ...
        return (R) mutableRegistry.add((RegistryKey)key, registry, lifecycle);
    }
    ...
    public Codec<T> getCodec() {
        Codec<Object> codec = Identifier.CODEC.flatXmap(identifier -> Optional.ofNullable(this.get(identifier)).map(DataResult::success).orElseGet(() -> DataResult.error("Unknown registry key in " + this.registryKey + ": " + identifier)), object -> this.getKey((T) object).map(RegistryKey::getValue).map(DataResult::success).orElseGet(() -> DataResult.error("Unknown registry element in " + this.registryKey + ":" + object)));
        Codec<Object> codec2 = Codecs.method_39511(object -> this.getKey((T) object).isPresent() ? this.getRawId((T) object) : -1, this::get, -1);
        return Codecs.method_39504((Codec)Codecs.method_39512(codec, codec2), this::getEntryLifecycle, object -> this.lifecycle);
    }
    ...
    public static <V, T extends V> T register(Registry<V> registry, RegistryKey<V> key, T entry) {
        return (T) ((MutableRegistry)registry).add(key, entry, Lifecycle.stable());
    }

    public static <V, T extends V> T register(Registry<V> registry, int rawId, String id, T entry) {
        return (T) ((MutableRegistry)registry).set(rawId, RegistryKey.of(registry.registryKey, new Identifier(id)), entry, Lifecycle.stable());
    }
