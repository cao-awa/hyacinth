...
implements NbtElementVisitor {
    ...
    public void visitList(NbtList element) {
        ...
        for (int mutableText = 0; mutableText < element.size(); ++mutableText) {
            ...
            i.append(new NbtTextFormatter(this.prefix, this.indentationLevel + 1).apply(element.get(mutableText)));
            if (mutableText != element.size() - 1) {
                i.append(ENTRY_SEPARATOR).append(this.prefix.isEmpty() ? SPACE : NEW_LINE);
            }
            string.append(i);
        }
        if (!this.prefix.isEmpty()) {
    ...
    public void visitCompound(NbtCompound compound) {
        ...
        while (iterator.hasNext()) {
            ...
            MutableText mutableText2 = new LiteralText(Strings.repeat(this.prefix, this.indentationLevel + 1)).append(NbtTextFormatter.escapeName(string)).append(KEY_VALUE_SEPARATOR).append(SPACE).append(new NbtTextFormatter(this.prefix, this.indentationLevel + 1).apply(compound.get(string)));
            if (iterator.hasNext()) {
