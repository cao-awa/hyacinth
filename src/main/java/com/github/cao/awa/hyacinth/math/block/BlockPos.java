package com.github.cao.awa.hyacinth.math.block;

import com.github.cao.awa.hyacinth.math.Mathematics;
import com.github.cao.awa.hyacinth.math.block.box.BlockBox;
import com.github.cao.awa.hyacinth.math.block.rotation.BlockRotation;
import com.github.cao.awa.hyacinth.math.box.Box;
import com.github.cao.awa.hyacinth.math.direction.AxisCycleDirection;
...
public class BlockPos
        extends Vec3i {
    public static final Codec<BlockPos> CODEC = Codec.INT_STREAM.comapFlatMap(stream -> EntrustParser.toArray(stream, 3).map(values -> new BlockPos(values[0], values[1], values[2])), pos -> IntStream.of(pos.getX(), pos.getY(), pos.getZ())).stable();
    ...
    public static Stream<BlockPos> stream(Box box) {
        return BlockPos.stream(Mathematics.floor(box.minX), Mathematics.floor(box.minY), Mathematics.floor(box.minZ), Mathematics.floor(box.maxX), Mathematics.floor(box.maxY), Mathematics.floor(box.maxZ));
    }
    ...
    static {
        SIZE_BITS_Z = SIZE_BITS_X = 1 + Mathematics.floorLog2(Mathematics.smallestEncompassingPowerOfTwo(30000000));
    ...
    public static class Mutable
        ...
        public Mutable(double d, double e, double f) {
            this(Mathematics.floor(d), Mathematics.floor(e), Mathematics.floor(f));
        }
        ...
        public Mutable set(double x, double y, double z) {
            return this.set(Mathematics.floor(x), Mathematics.floor(y), Mathematics.floor(z));
        }
        ...
        public Mutable clamp(Direction.Axis axis, int min, int max) {
            switch (axis) {
                case X -> {
                    return this.set(Mathematics.clamp(this.getX(), min, max), this.getY(), this.getZ());
                }
                case Y -> {
                    return this.set(this.getX(), Mathematics.clamp(this.getY(), min, max), this.getZ());
                }
                case Z -> {
                    return this.set(this.getX(), this.getY(), Mathematics.clamp(this.getZ(), min, max));
                }
            }
