...
public class Vec3d implements Position {
     ...
     *
     * @param rgb
     *         the color in the 0xRRGGBB format
     ...
     *
     * @param polar
     *         the vector composed of pitch and yaw
    ...
    public static Vec3d fromPolar(float pitch, float yaw) {
        float f = Mathematics.cos(- yaw * ((float) Math.PI / 180) - (float) Math.PI);
        float g = Mathematics.sin(- yaw * ((float) Math.PI / 180) - (float) Math.PI);
        float h = - Mathematics.cos(- pitch * ((float) Math.PI / 180));
        float i = Mathematics.sin(- pitch * ((float) Math.PI / 180));
     ...
     *
     * @param delta
     *         the interpolation coefficient in the range between 0 and 1
     * @param to
     *         the vector to interpolate to
     */
    public Vec3d lerp(Vec3d to, double delta) {
        return new Vec3d(Mathematics.lerp(delta, this.x, to.x), Mathematics.lerp(delta, this.y, to.y), Mathematics.lerp(delta, this.z, to.z));
    }
    ...
    public Vec3d rotateX(float angle) {
        float f = Mathematics.cos(angle);
        float g = Mathematics.sin(angle);
        double d = this.x;
    ...
    public Vec3d floorAlongAxes(EnumSet<Direction.Axis> axes) {
        double d = axes.contains(Direction.Axis.X) ? (double) Mathematics.floor(this.x) : this.x;
        double e = axes.contains(Direction.Axis.Y) ? (double) Mathematics.floor(this.y) : this.y;
        double f = axes.contains(Direction.Axis.Z) ? (double) Mathematics.floor(this.z) : this.z;
        return new Vec3d(d, e, f);
    }
